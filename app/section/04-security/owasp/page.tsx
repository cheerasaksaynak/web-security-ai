        ...
        <div className="max-w-6xl mx-auto px-6">
          <div className="text-center mb-14">
            <h2 className="text-3xl font-black text-slate-900 mb-4">
              <span className="text-sky-600">A03</span> — SQL Injection
            </h2>
            <div className="w-16 h-1 bg-sky-500 rounded-full mx-auto mb-4"></div>
            <p className="text-slate-600">การโจมตีที่ผู้ใช้แทรก SQL command ผ่าน input เพื่อเข้าถึงหรือแก้ไขข้อมูล</p>
          </div>

          {/* How it works */}
          <div className="bg-white rounded-2xl p-8 mb-8 border border-slate-200">
            <h3 className="font-bold text-slate-900 text-xl mb-6">
              <i className="fas fa-question-circle text-sky-600 mr-2"></i>
              ทำงานอย่างไร?
            </h3>
            <div className="grid md:grid-cols-3 gap-6 mb-6">
              {[
                ...
                <div key={s.step} className="text-center">
                  ...
                  <code className="text-red-600 text-xs font-mono">{s.desc}</code>
                </div>
              ))}
            </div>
          </div>
...
// กฎ: ห้าม string concatenation ใน SQL เด็ดขาด!`}
          ...
          <div className="grid md:grid-cols-3 gap-4">
            {[
              ...
              <div key={tip.title} className="bg-emerald-50 border border-emerald-200 rounded-xl p-4">
                <i className={`${tip.icon} text-emerald-600 mb-2 block`}></i>
                <p className="font-semibold text-emerald-700 text-sm mb-1">{tip.title}</p>
            ...
            <div key={xss.type} className={`bg-white border-t-4 ${xss.color} rounded-2xl overflow-hidden border border-slate-200`}>
              ...
              <div className="p-5 space-y-3">
                ...
                <div className="bg-slate-100 rounded-lg p-3">
                  <code className="text-slate-700 text-xs font-mono break-all">{xss.example}</code>
                </div>
            ...
            <div className="bg-white dark:bg-slate-800 rounded-2xl p-6 border border-slate-200 dark:border-slate-700">
              ...
              <div className="space-y-3">
                {[
                  ...
                  <div key={item.title} className="flex gap-3 p-3 bg-sky-50 dark:bg-sky-950/30 rounded-xl border border-sky-100 dark:border-sky-800">
        ...
        <WorkshopBox
          ...
          steps={[
            ...
            },
            {
              ...
              description: 'แต่ละ bug คือ SQLi, XSS, Auth failure, หรือ exposed credentials?',
            },
            {
              step: 3,
              title: 'ใช้ Copilot Review',
              code: 'Review this code for security vulnerabilities.\nIdentify each issue, explain the risk, and suggest a secure fix.\nAlso check for any hardcoded credentials or secrets that should be moved to environment variables.',
            },
            {
              step: 4,
          ...
          copilotPrompt="Review this code for security vulnerabilities. Identify each issue, categorize it (SQL Injection / XSS / CSRF / Auth / Exposed Credentials), explain the risk, and provide a secure fix with explanation. Also check whether any API keys, passwords, or secrets are hardcoded in the source code and suggest moving them to environment variables."
        />
      </section>
      ...
      <div className="max-w-6xl mx-auto px-6 py-10">
        <div className="flex items-center justify-between">
          <Link href="/section/04-security/server-config" className="flex items-center gap-2 text-slate-500 hover:text-sky-500 transition-colors text-sm font-medium">
            <i className="fas fa-arrow-left text-xs"></i> Server Config
          </Link>
          <Link href="/section/05-ai" className="flex items-center gap-2 text-slate-500 hover:text-sky-500 transition-colors text-sm font-medium">
            05 AI Coding Agent <i className="fas fa-arrow-right text-xs"></i>
          </Link>
        </div>
      </div>
    </div>
